---
title: "Environment Variables Reference for AgentSwarm"
...
| Variable | Description | Default |
|---|---|---|
| `CODEX_RUNTIME_IMAGE` | Image used for automated Codex task runs. | `agentswarm-agent-runtime-codex:latest` |
| `CLAUDE_RUNTIME_IMAGE` | Image used for automated Claude task runs. | `agentswarm-agent-runtime-claude:latest` |
| `GIT_TERMINAL_IMAGE` | Restricted Alpine image with git, vim, and diff3 for in-browser Git terminals. | `local/git-terminal:latest` |
| `CODEX_INTERACTIVE_IMAGE` | Image for in-browser interactive Codex terminals. | `local/codex-interactive:latest` |
| `CLAUDE_INTERACTIVE_IMAGE` | Image for in-browser interactive Claude Code terminals. | `local/claude-interactive:latest` |

<Note>
  Setting any terminal image variable to an empty string disables that terminal type entirely. Runtime images are built locally by `./agentswarm.sh init` or `./agentswarm.sh rebuild` — they are not pulled from a registry.
</Note>
